...
r.route({ a: 'x' }, {
  pre: [ppre],
  post: [ppost],
  children: [[{ a: 'x', b: 'y' }, { pre: [cpre], handler, post: [cpost] }]]
});
...
- Order is parent pre -> child pre -> handler -> child post -> parent post.
- calls -> `['ppre:xy', 'cpre:xy', 'handler:xy', 'cpost:xy', 'ppost:xy']`; `scope.result` -> `'HC'`
...
r.route({ a: 'x' }, {
  ...
  children: [[
    // Child adds a stage-specific handler that handles and stops
    { a: 'x' }, { onPreError: [({ error }) => { calls.push('child:onPreError:' + error.message); return { result: 'ERR' }; }] }
  ]]
});

const { scope } = await r.request({ subject: 'x' });
// calls -> ['ppre', 'child:onPreError:E1']
// scope.result === 'ERR'; scope.error is Error('E1'); parent:onError not reached.
```
