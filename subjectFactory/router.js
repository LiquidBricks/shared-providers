    ...
    async request(arg = {}) {
      ...
      if (typeof best.handler === 'function') {
        ...
        const runErrorHandlers = async ({ stage, index, failingFn, error }) => {
          ...
          // For safety and predictability, stringify the failing function when invoking router-level handler
          if (typeof onError === 'function') chain.push(async (args) => {
            const safeArgs = args && typeof args === 'object'
              ? { ...args, fn: args.fn !== undefined ? String(args.fn) : args.fn }
              : args
            return onError(safeArgs)
          })
          let lastError = error
